...
PageDirBase1	equ	00210000h	; 页目录开始地址:	2M + 64K
PageTblBase1	equ	00211000h	; 页表开始地址:		2M + 64K + 4K
...
.2:
	...
	mov	ebx, 4096
	mul	ebx
	mov	ecx, eax    ; ECX: LinearAddrDemo 对应那个页表
	mov	eax, LinearAddrDemo
	shr	eax, 12
	and	eax, 03FFh	; 1111111111b (10 bits)
	mov	ebx, 4      
	mul	ebx         ; EAX: LinearAddrDemo 在页表内的偏移
	add	eax, ecx
	add	eax, PageTblBase1
    ; 此时[es:eax]即是LinearAddrDemo地址对应的页地址，将其改到ProcBar
...
.loop:					    ;{  // 每次得到一个ARDS(Address Range Descriptor Structure)结构
	...
	mov	edi, ARDStruct		;	{	// 依次显示：BaseAddrLow，BaseAddrHigh，
.1:					        ;       // LengthLow，LengthHigh，Type
	push	dword [esi]		;
	...
	jne	.2				    ;   {   // AddressRangeMemory : 1, AddressRangeReserved : 2
