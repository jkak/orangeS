...
.2:	        ; 显示完毕
    ...
    ltr ax  ; 在任务内发生特权级变换时要切换堆栈，
            ; 而内层堆栈的指针存放在当前任务的TSS中，
            ; 所以要设置任务状态段寄存器 TR。
  
    ;;;; use retf goto low level PL code
    push    SelectorStack3      ; ESS
    push    TopOfStack3         ; ESP
    push    SelectorCodeRing3   ; ECS
    push    0                   ; EIP
...
LABEL_CODE_RING3:
    ...
    ; call 调用的门目标代码，继续jmp到LDT的codeA代码
    ; 并转移到16位代码，以返回dos。故下面的死循环并未执行
